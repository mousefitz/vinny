...
async def get_intent_from_prompt(bot_instance, message):
    ...
    intent_prompt = (
        ...
        "## Available Functions:\n"
        "1. `generate_image`: For generic art requests NOT about a specific user (e.g. 'paint a cat').\n"
        "2. `generate_user_portrait`: For requests to paint A SPECIFIC PERSON in the chat (including the user themselves).\n"
        "   - `target`: The name or reference of the person to paint (e.g. 'me', 'myself', 'alex', 'vinny', 'him').\n"
        "   - `details`: Any specific visual modifiers (e.g. 'wearing a track suit', 'as a jedi', 'eating pizza').\n"
        ...
        "5. `tag_user`: For requests to ping someone.\n"
        ...
        "- 'draw alex as a clown' -> {\"intent\": \"generate_user_portrait\", \"args\": {\"target\": \"alex\", \"details\": \"as a clown\"}}\n"
